...
type ModalReducer<T> = (state: ModalState<T>, action: ModalAction<T>) => ModalState<T>;
...
export const createModalReducer = <T extends unknown>(
  ...
  const reducer = (state: ModalState<T> = initialState, action: ModalAction<T>): ModalState<T> => {
    switch (action.type) {
      case SHOW_MODAL: {
        return {
          ...state,
...
export const usePopupReducer = <T extends unknown>(
  defaultModalState?: Partial<ModalState<T>>,
): ModalHook<T> => {
