...
describe('handleResponse', () => {
  ...
  it('should logout when getToken does not return token in doFetchUserProfile', async () => {
    const generator = saga.doFetchUserProfile();
    await act(async () => {
      const apiCallEffect = generator.next().value;
      expect(apiCallEffect).toEqual(call(apiCall, USER_PROFILE));
    });
  });
  ...
  it('should return correct query string with only required parameters', () => {
    const filter : any = {
      order: 'name',
      page: 1,
      limit: 10,
